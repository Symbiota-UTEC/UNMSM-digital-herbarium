...
`App.tsx` owns every route. Pages never call `useNavigate`: they receive `onNavigate(page, params)` as a prop, and `buildRoute()` maps `(page, params)` to a path + router `state`. **`state` carries navigation context only** (`returnTo`, `originReturnTo`, `returnOccurrenceId`, `restoreSearch`, the `collectionId` to go back to, `taxonId`); it can be missing (direct link, new tab, bookmark), so a page must work from the URL id alone. Data and permissions come from the API: `GET /collections/{id}` and the occurrence's `collection` return the name plus `canEdit`/`canManage`, which the backend computes — never derive permissions in the client nor pass them through `state`. Routes with URL params (`/collections/:id`, `/collections/:id/occurrences/new`, `/occurrences/:id`, …) use small wrapper components defined **at module level** in `App.tsx` that read `useParams()`/`useLocation().state` and forward them as props.
