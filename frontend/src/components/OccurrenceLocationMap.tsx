...
export function OccurrenceLocationMap({ lat, lon, footprintWKT, uncertaintyMeters }: Props) {
  ...
  const point: [number, number] | null = lat != null && lon != null ? [lon, lat] : representativePoint(polygon);
  const circleMeters = !polygon && uncertaintyMeters != null && uncertaintyMeters > 0 ? uncertaintyMeters : null;

  if (!point) {
    return (
      <p className="text-sm text-muted-foreground">Esta ocurrencia no tiene coordenadas ni polígono registrados.</p>
    );
  }

  const caption = polygon
